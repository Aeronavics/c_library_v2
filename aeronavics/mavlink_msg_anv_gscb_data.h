 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_anv_gscb_data_pack_status(uint8_t system_id, uint8_t component_id, mavlink_status_t *_status, mavlink_message_t* msg,
                               uint8_t active_profile, uint8_t battery_1_enabled, uint8_t battery_2_enabled, uint8_t external_power_enabled, uint8_t tray_fan_enabled, uint8_t electronics_fan_enabled)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_ANV_GSCB_DATA_LEN];
    ...
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_ANV_GSCB_DATA_MIN_LEN, MAVLINK_MSG_ID_ANV_GSCB_DATA_LEN, MAVLINK_MSG_ID_ANV_GSCB_DATA_CRC);
#else
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_ANV_GSCB_DATA_MIN_LEN, MAVLINK_MSG_ID_ANV_GSCB_DATA_LEN);
#endif
}
...
/**
 * @brief Encode a anv_gscb_data struct with provided status structure
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param status MAVLink status structure
 ...
 * @param anv_gscb_data C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_anv_gscb_data_encode_status(uint8_t system_id, uint8_t component_id, mavlink_status_t* _status, mavlink_message_t* msg, const mavlink_anv_gscb_data_t* anv_gscb_data)
{
    return mavlink_msg_anv_gscb_data_pack_status(system_id, component_id, _status, msg,  anv_gscb_data->active_profile, anv_gscb_data->battery_1_enabled, anv_gscb_data->battery_2_enabled, anv_gscb_data->external_power_enabled, anv_gscb_data->tray_fan_enabled, anv_gscb_data->electronics_fan_enabled);
}
