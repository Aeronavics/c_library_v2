 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_anv_gs_ctrl_data_pack_status(uint8_t system_id, uint8_t component_id, mavlink_status_t *_status, mavlink_message_t* msg,
                               uint8_t controller_id, const uint32_t *an_channel, uint32_t dig_channel)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_ANV_GS_CTRL_DATA_LEN];
    ...
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_ANV_GS_CTRL_DATA_MIN_LEN, MAVLINK_MSG_ID_ANV_GS_CTRL_DATA_LEN, MAVLINK_MSG_ID_ANV_GS_CTRL_DATA_CRC);
#else
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_ANV_GS_CTRL_DATA_MIN_LEN, MAVLINK_MSG_ID_ANV_GS_CTRL_DATA_LEN);
#endif
}
...
/**
 * @brief Encode a anv_gs_ctrl_data struct with provided status structure
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param status MAVLink status structure
 ...
 * @param anv_gs_ctrl_data C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_anv_gs_ctrl_data_encode_status(uint8_t system_id, uint8_t component_id, mavlink_status_t* _status, mavlink_message_t* msg, const mavlink_anv_gs_ctrl_data_t* anv_gs_ctrl_data)
{
    return mavlink_msg_anv_gs_ctrl_data_pack_status(system_id, component_id, _status, msg,  anv_gs_ctrl_data->controller_id, anv_gs_ctrl_data->an_channel, anv_gs_ctrl_data->dig_channel);
}
